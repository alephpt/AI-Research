...
void validConvolution(CNNFeature* output, CNNFeature* input, Kernel* k, int stride) {
	...
	output->values = fmatrix(output->height, vector<float>(output->width, 0.0f));
	fmatrix cached = fmatrix(kernel_rows, vector<float>(kernel_cols, 0.0f));
	...
	for (int i = 0; i < output->height; ++i) {
		...
		cached = fmatrix(kernel_rows, vector<float>(kernel_cols, 0.0f));
	}
}
